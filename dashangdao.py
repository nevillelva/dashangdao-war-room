...
# 必須在第一行執行
st.set_page_config(layout="wide", page_title="54088 - 終極大腦 V16.4", initial_sidebar_state="expanded")
    ...
    try:
        ...
        if hist.empty or current_price <= 0:
            return {
                ...
                "tactical_summary": "📡 系統目前無法連線取得報價，建議暫時在旁觀望。", "is_high_yield": False, "is_cyclical": False, "vol_ratio": 0.0, "diff_from_cost": 0.0, "vol": 0
            }
        ...
        tactical_summary = "目前走勢進入混沌期，多空交戰中，建議在旁保留現金觀望。"
        ...
        if is_fake_breakout: 
            ...
            tactical_summary = "❌ 偵測到明顯假突破！上方套牢賣壓極重，這是主力慣用的誘多手法，嚴禁追高！"
            ...
            is_shield_active = False; is_action_needed = True
            tactical_summary = "❌ 虧損已達 10% 絕對紀律界線！請立刻市價全數平倉，保護核心本金！"
            ...
            if is_shield_active and gain < -2.0: 
                ...
                tactical_summary = "⏳ 雖然整體動能向上，但現在追高風險過大，請等量縮拉回均線再佈局。"
...
with st.sidebar:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_code):
                ...
                if d and "❌" not in d['signal'] and d['price'] > 0:
                    ...
                    elif mode == "stealth" and d['vol_ratio'] >= 1.5 and d['diff_from_cost'] <= 15.0: results.append(d)
                    elif mode == "yield" and (d['is_high_yield'] or d['is_cyclical']): results.append(d)
        return results
    ...
    if st.button("🕵️‍♂️ 魚頭潛伏與轉機", use_container_width=True):
        with st.spinner("篩選長線低檔打底大機率暴動標的..."):
            st.session_state.scan_results = run_parallel_scan(FULL_MARKET_CODES, "stealth", is_panic)
            st.session_state.scan_mode = "stealth"; st.rerun()
    if st.button("🛡️ 總經防禦與景氣循環", use_container_width=True):
        with st.spinner("篩選高殖利率與深度循環防守標的..."):
            st.session_state.scan_results = run_parallel_scan(FULL_MARKET_CODES, "yield", is_panic)
...
if scan_mode_current := st.session_state.get('scan_mode', ""):
    ...
    if not st.session_state.scan_results:
        st.warning("⚠️ 雷達掃描完畢，未發現符合該型態條件的標的！系統已過濾風險過高之死魚股。")
    else:
        cols = st.columns(2)
