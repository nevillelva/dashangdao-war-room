...
st.set_page_config(layout="wide", page_title="54088 - 終極大腦 V15.4")
...
st.markdown(f"<div style='text-align:right; color:#888; font-size:12px; margin-bottom:10px;'>大盤天候：<strong style='color:{weather_color};'>{weather_str}</strong> | V15.4 全市場解鎖版 | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</div>", unsafe_allow_html=True)
...
def run_parallel_scan(target_codes, mode):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
        ...
        for i, future in enumerate(concurrent.futures.as_completed(future_to_code)):
            ...
            status_text.markdown(f"📡 全市場雷達深度掃描中 ({i+1}/{len(target_codes)})... [即時過濾: 阻擋 {api_fails} | 太貴 {expensive_blocks}]")
...
with col_scan1:
    ...
    if st.button("🚀 黃金起漲與魚身掃描\n(均線多頭或帶量突破)", use_container_width=True):
        st.markdown("</div>", unsafe_allow_html=True)
        with st.spinner("📡 正在對全市場 1700+ 檔進行深度掃描，這將耗時約 2~4 分鐘，請總指揮耐心等候..."):
...
# [V15.4 破除 UI 隱蔽的幽靈 Bug：即使是 0 檔也會顯示報告]
