...
st.set_page_config(layout="wide", page_title="54088 - V11.1")
...
def calculate_tactical_signals(symbol_data, category_type="main", mode="短線技術動能單", manual_target=0.0, portfolio_data=None):
    try:
        ...
        if opt_event_vanish: signal_text, color_border, signal_bg = f"{ACTION_NO} 買進核心理由消失，霸王逃命條款觸發，強制撤退。", "#e74c3c", "#3a1515"; is_shield_active = False
            ...
            if current_price <= buy_high: signal_text, color_border, signal_bg = f"{ACTION_YES} 融資斷頭潮來臨！股價已達便宜區，恐慌中浮現超額價值，長線重壓！", "#00FF00", "#153a20"
            else: signal_text, color_border, signal_bg = f"{ACTION_WAIT} 大盤恐慌下殺中！但此檔尚未跌入下方【最佳入場區】，請耐心等斷頭賣壓把它打下來再撿！", "#f39c12", "#3a3015"
...
st.markdown(f"<div style='text-align:right; color:#888; font-size:12px; margin-bottom:10px;'>大盤天候：<strong style='color:{weather_color};'>{weather_str}</strong> | V11.1 霸王修正版 | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</div>", unsafe_allow_html=True)
...
# [V11.1 修復：將三大雷達全面改回 for 迴圈，避免 NameError 且恢復進度條]
...
def render_stock_card(d, ui_key_prefix, is_portfolio=False, p_data=None):
    ...
    # [V11.1 修復：完美還原 V7.1 的 Tooltip 結構與標籤顯示]
