...
def _roc_date_to_display(date_str):
    """
    【V160 新增】TWSE 這個端點回傳的日期是民國年 YYYMMDD（例：'1150729' = 民國115年7月29日
    = 西元2026-07-29），不是西元年。轉成好讀的西元日期字串；格式不對就照原樣回傳，不猜。
    """
...
with st.expander("🤖 系統自主選股模擬倉（做多 vs 做空 勝率PK）", expanded=False):
    ...
    if st.button("▶️ 立即執行一次系統選股", key="run_sys_pick", use_container_width=True):
        # 【V160 修復】原本直接用今天日期，導致在週六日手動測試時，
        # 建倉日會寫成 07/18(六)、07/19(日) 這種根本沒開盤的日子，
        # 之後回測/勝率統計拿這個日期去對股價會對不上。改用最近交易日。
        _run_date = get_current_or_last_trading_date()
        # 【V160 修復】改用真正的全市場池，不再退回雷達/持倉/觀察區。
        # 之前的版本在候選池為空時會退回你自己關注的清單，這樣測出來的「系統選股」
        # 其實是在你已經篩過的股票裡選，不是真正的全市場掃描，勝率比較就失去客觀性，
        # 跟系統定位「全自動全市場」矛盾。現在統一用側邊欄「全市場掃描池大小」設定的範圍。
        _pool = GLOBAL_MARKET_CODES[:scan_pool_size]
        if not _pool:
            st.warning("全市場代碼清單目前是空的，請確認 TW_STOCK_NAMES 資料源正常。")
        else:
            with st.spinner(f"掃描池 {len(_pool)} 檔（側邊欄可調整範圍）中，挑選多空候選中（檔數越多耗時越久）..."):
                ...
                sb_log_system_run(_run_date, 'manual_test', len(_longs) + len(_shorts),
                                  ...
                                  f"全市場手動測試（池{len(_pool)}檔）：多{len(_long_entries)}檔/空{len(_short_entries)}檔")
            st.success(f"✅ 全市場掃描完成（{len(_pool)}檔）：做多 {len(_long_entries)} 檔、做空 {len(_short_entries)} 檔已進場模擬倉")
            ...
            if _sym_q:
                ...
                if not _cl and not _hd:
                    ...
                    if _cl:
                        ...
                        } for r in _cl]), use_container_width=True, hide_index=True)
                    if _hd:
                        st.markdown("**持倉中**")
                        st.dataframe(pd.DataFrame([{
                            '方向': '🔴做多' if r.get('side') == 'long' else '🔵做空',
...
def render_action_buttons(card, code, is_portfolio, section_key='pinned_stocks'):
    ...
    with st.expander("⚙️ 資料校正、人工覆寫與 AI 推演", expanded=False):
        ...
        if st.button("📈 顯示K線圖", key=f"kline_btn_{code}{btn_suffix}", use_container_width=True):
            st.session_state[f'show_kline_{code}'] = not st.session_state.get(f'show_kline_{code}', False)
        if st.session_state.get(f'show_kline_{code}'):
            with st.spinner("繪製K線圖中..."):
                _khist, _ = get_real_stock_data_yfinance(code)
                render_kline_chart(code, _khist)
