...
st.set_page_config(layout="wide", page_title="54088 - 終極大腦 V15.3")
...
st.markdown(f"<div style='text-align:right; color:#888; font-size:12px; margin-bottom:10px;'>大盤天候：<strong style='color:{weather_color};'>{weather_str}</strong> | V15.3 終極抗壓版 | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</div>", unsafe_allow_html=True)
...
# [V15.3 終極修正版：防封鎖排隊發送 與 深度過濾報告回傳]
def run_parallel_scan(target_codes, mode):
    results = []
    api_fails, expensive_blocks, no_signal_blocks = 0, 0, 0
    
    # 再次降低併發數防封鎖，採用排隊延遲發送
    with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
        future_to_code = {}
        for c in target_codes:
            future_to_code[executor.submit(calculate_tactical_signals, f"{c}:?:?:?:?", "scan")] = c
            time.sleep(0.05) # 🚦 戰術級防封鎖：每個執行緒發送間隔，避免瞬間 DDOS Yahoo
        ...
        for i, future in enumerate(concurrent.futures.as_completed(future_to_code)):
            d = future.result()
            if d:
                # 統計被過濾掉的原因，讓總指揮不會覺得在盲測
                if "API抓取失敗" in d['signal']:
                    api_fails += 1
                elif "股價太貴" in d['signal']:
                    expensive_blocks += 1
                else:
                    is_match = False
            ...
            status_text.markdown(f"📡 掃描中 ({i+1}/{len(target_codes)})... [過濾報告即時統計: 阻擋 {api_fails} | 太貴 {expensive_blocks}]")
            
    status_text.empty()
    progress_bar.empty()
    
    # 將統計數據打包回傳至前端
    st.session_state.scan_stats = {"api_fails": api_fails, "expensive_blocks": expensive_blocks, "no_signal": no_signal_blocks, "total": len(target_codes)}
    return results
...
with col_scan1:
    ...
    if st.button("🚀 黃金起漲與魚身掃描\n(均線多頭或帶量突破)", use_container_width=True):
        st.markdown("</div>", unsafe_allow_html=True)
        st.session_state.scan_results = run_parallel_scan(FULL_MARKET_CODES[:500], "golden")
...
with col_scan2:
    ...
    if st.button("🕵️‍♂️ 魚頭潛伏與本質變化\n(長線底/底部爆量)", use_container_width=True):
        st.markdown("</div>", unsafe_allow_html=True)
        st.session_state.scan_results = run_parallel_scan(FULL_MARKET_CODES[:500], "stealth")
...
with col_scan3:
    ...
    if st.button("🛡️ 總經防禦與景氣循環\n(高殖利/深循環/量大於300)", use_container_width=True):
        ...
        with st.spinner("📡 正在即時生成全市場【高息防禦與深度循環】動態清單..."):
            dynamic_pool = [c for c, data in FUNDAMENTAL_DB.items() if data['Yield'] >= 5.0 or data['PE'] < 12.0 or data['PB'] < 1.2][:500]
...
# [V15.3 破除 UI 隱蔽的幽靈 Bug：即使是 0 檔也會顯示報告]
...
if scan_mode_current != "":
    ...
    if len(st.session_state.scan_results) == 0:
        ...
        <p style='color:#ddd; font-size:14px;'>總指揮，這不是系統當機，也不是按鈕壞掉。當結果為 0 檔時，以下是系統自動生成的 <strong>【戰情室深度過濾報告】</strong>：</p>
        <ul style='color:#aaa; font-size:14px; line-height: 1.8;'>
            <li>📡 掃描總池：<strong style='color:#fff;'>{stats.get('total', 0)}</strong> 檔</li>
            <li>🚨 <span style='color:#e74c3c;'>遭 Yahoo 防爬蟲阻擋 (斷線)：<strong>{stats.get('api_fails', 0)}</strong> 檔</span> ➔ <small>若此數字極高，代表 IP 已被 Yahoo 暫時封鎖，請等 5~10 分鐘後再試。</small></li>
