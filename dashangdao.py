...
for i, s in enumerate(STOCKS):
    with cols[i % 2]:
        st.markdown(f'''<div class="card">
            ...
            <div style="font-size:13px; color:#aaa; margin-top:5px;">建議進價: {s['buy']} | 波動限制: {s['v']}</div>
            <div style="font-size:13px; color:#aaa;">{"🛑 護城河健在" if s['shd']>=3 else "⚠️ 護城河脆弱"}</div>
        </div>''', unsafe_allow_html=True)
        
        # 重新植入模擬持倉組件
        with st.expander(f"💼 {s['n']} 模擬持倉精算"):
            cost = st.number_input(f"成本價 {s['c']}", value=370.0, key=f"c_{s['c']}")
            ...
            if st.button("❌ 一鍵清空今日標的", key=f"del_{s['c']}"):
                st.rerun()
