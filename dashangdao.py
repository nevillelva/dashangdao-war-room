...
# 1. 基礎配置與全域變數初始化
# ==========================================
st.set_page_config(layout="wide", page_title="54088 戰情室 V129.9", initial_sidebar_state="expanded")
st.toast("✅ [系統提示] V129.9 記憶體單機重構與順序修復版 啟動成功！")
    ...
    if os.path.exists(USER_DB_FILE):
        try:
            with open(USER_DB_FILE, "r", encoding="utf-8") as f:
                ...
                if "inst_history" in data: st.session_state.inst_history = data["inst_history"]
...
def fetch_stock_names():
    ...
    fallbacks = {"2330":"台積電", "2303":"聯電", "2317":"鴻海", "8182":"加高", "1519":"華城", "1227":"佳格"}
...
# ==========================================
# 4. 夜間手動派班模組 (一鍵打包全市場法人)
# ==========================================
def run_nightly_institutional_batch():
    inst_db = {}
    with st.spinner("連線至證交所與櫃買中心打包全市場法人數據..."):
        try:
...
def get_finmind_and_deep_fundamentals(symbol, token_string, curr_price):
    ...
    # 既然有免費額度限制，且基本面變化不大，這段 FinMind 備用查詢保留即可，若額度爆了會回傳預設值，不影響系統運作
...
def fetch_recent_chips_rescue(symbol):
    # 若在單檔運算時缺乏歷史資料，做單次安全獲取 (不燒 FinMind)
...
def draw_card(d, ui_key_prefix, is_portfolio=False, p_data=None):
    ...
    if rev_val is None or abs(rev_val) < 0.01: rev_display = "<span style='color:#888;'>API未提供</span>"
    else: rev_display = f"{rev_val:.1f}%"
...
with st.sidebar:
    ...
    st.write("取代不穩定的雲端連線，請於盤後 (21:30) 執行以下 1-2 步以保全今日戰情。隔日開盤前請執行第 3 步還原記憶。")
    ...
    if st.session_state.inst_history:
        ...
        st.markdown("<div style='font-size:13px; color:#ff4d4d; margin-top:10px;'>⚠️ 警告：系統目前無任何法人歷史記憶，大戶連買/連賣訊號將失效！請點擊上方按鈕抓取或上傳備份檔。</div>", unsafe_allow_html=True)
...
# ==========================================
# 9. 畫面主架構渲染 (模擬倉、雷達、掃描結果)
# ==========================================
col_nav1, col_nav2 = st.columns([8, 2])
with col_nav1: st.markdown("<h1 style='color:#FFB300; margin: 0;'>🚀 54088 戰情室 V129.9</h1>", unsafe_allow_html=True)
