...
# 基礎配置與狀態初始化
# ==========================================
st.set_page_config(layout="wide", page_title="54088 - 戰情室 V82.0", initial_sidebar_state="expanded")
...
.tag-gold { background: #3a2e15; padding: 4px 8px; border-radius: 4px; font-size: 13px; color: #f1c40f; border: 1px solid #f1c40f; display: inline-block; margin: 0 5px 5px 0; font-weight: bold; }
...
CONGLOMERATES = {
    ...
    "台積電集團": ["2330", "5347", "3443", "6789", "3374", "3443"],
    "威盛集團": ["2388", "2498", "3169", "4967", "6598", "2308"]
}
...
def get_market_weather():
    try:
        ...
        display_str = f"上市: {c_idx:,.0f} ({twii_gain:+.2f}%) | 上櫃: {two_curr:,.2f} ({two_gain:+.2f}%)"
...
def calculate_signals(symbol, data_tuple, portfolio_data=None, is_panic_global=False, twii_gain=0.0, is_scan=False):
    ...
    ai_tags = []
    group_tagged = False
    for g_name, codes in CONGLOMERATES.items():
        if symbol in codes:
            ai_tags.append(f"J. {g_name}")
            group_tagged = True
            break
    if t_buy >= 400:
        ai_tags.append("K. 投信作帳")
    ...
    chip_text = ""
    if chip_conc > 0:
        chip_text += f"<br><span style='color:#ccc;'>D. 籌碼流向：法人單日買超 {f_buy+t_buy:,} 張 (集中度 {chip_conc:.1f}%)</span>"
...
# ==========================================
with st.sidebar:
    if st.button("[閃電強制全域更新]", help="立刻清除快取，抓取最新大盤與個股即時數據", use_container_width=True, type="primary"):
        ...
        if not fm_tokens:
            ...
            for i, token in enumerate(fm_tokens):
                ...
                try:
                    ...
                    st.markdown(f"<div>[金鑰 #{i+1}] <span class='key-status-fail'>連線逾時或錯誤</span></div>", unsafe_allow_html=True)
        ...
        for i, c in enumerate(codes):
            ...
            if d and d['vol_5d'] >= min_vol and not d['is_action_needed']: 
                if d['signal'] not in invalid_signals:
                    ...
                    elif cmd_name == "指令五" and (d.get('chip_conc', 0) >= 8.0 or d.get('f_consec', 0) >= 3 or d.get('t_consec', 0) >= 3 or (d.get('f_buy',0) + d.get('t_buy',0) >= 800)): results.append(d)
    ...
    if st.button("[指令四] 作帳行情 (集團/投信)", help="【季度決戰部隊】\n鎖定六大集團與投信重倉買超的標的。\n抓出每季末大戶結算前拉抬績效的作帳黑馬。", use_container_width=True):
        raw_results = run_command_scan("指令四", scan_scope, min_volume_filter)
        st.session_state.ai_report = generate_ai_report("指令四：作帳行情", raw_results)
        st.session_state.scan_results = raw_results
        st.session_state.scan_mode = "cmd_4"
        
    if st.button("[指令五] 籌碼霸王色 (異常吸籌)", help="【主力現形部隊】\n嚴選單日籌碼集中度突破，或外資投信連續異常買超的標的。\n抓出背後有大人偷偷囤貨的潛力飆股。", use_container_width=True):
...
with col_nav1: st.markdown("<h1 style='color:#FFB300; margin: 0;'>54088 戰情室 V82.0 (籌碼記憶矩陣版)</h1>", unsafe_allow_html=True)
with col_nav2:
...
if now_month in [1, 2, 3]:
    quarter_info = "當前戰局：第一季 (Q1 作帳戰術期) | 提前佈局黃金窗：2月中旬至3月初 | 決戰平倉撤退線：3月底前最後四天"
elif now_month in [4, 5, 6]:
    quarter_info = "當前戰局：第二季 (Q2 半年報主戰場) | 提前佈局黃金窗：5月中旬至6月初 | 決戰平倉撤退線：6月底前最後四天"
elif now_month in [7, 8, 9]:
    quarter_info = "當前戰局：第三季 (Q3 季末作帳伏擊期) | 提前佈局黃金窗：8月中旬至9月初 | 決戰平倉撤退線：9月底前最後四天"
else:
    quarter_info = "當前戰局：第四季 (Q4 終極年報作帳與作夢行情) | 提前佈局黃金窗：11月中旬至12月初 | 決戰平倉撤退線：12月底前最後四天"
...
<div style='background:#1c241d; padding:10px; border-radius:5px; border-left:3px solid #f1c40f; margin-bottom:10px; font-size:13px; color:#f1c40f; font-weight:bold;'>
[戰術作帳指引] {quarter_info}
</div>
...
if st.session_state.pinned_stocks:
    ...
    with c_s2:
        available_radar_tags = [
            ...
            "J. 華新集團", "J. 國巨集團", "J. 鴻海集團", "J. 聯電集團", "J. 台積電集團", "J. 威盛集團", "K. 投信作帳",
            "L. 外資連買", "M. 投信連買", "N. 異常大量吸籌"
        ]
...
if scan_mode_current := st.session_state.get('scan_mode', ""):
    ...
    selected_tags = st.multiselect("[動態二次過濾] (可多選，即時篩選無須重新掃描)：", available_tags, placeholder="選擇您要的籌碼標籤進行精準狙擊...")
    st.markdown("</div>", unsafe_allow_html=True)
