    ...
    for model_id in get_nim_models():
        try:
            completion = client.chat.completions.create(
                ...
                temperature=0.3, max_tokens=1500, timeout=15
            )
        ...
        try:
            completion = client.chat.completions.create(
                ...
                messages=[{"role": "system", "content": "你是一位冷血的台灣股市操盤幕僚。所有輸出嚴格使用繁體中文，並使用台灣金融專有名詞。直擊核心。"},
                          {"role": "user", "content": prompt}],
                temperature=0.2, max_tokens=1200, timeout=15
            )
            ...
            if '404' in emsg or 'not found' in emsg or 'does not exist' in emsg:
                ...
                errors.append(f"{short}: 連線逾時(15s)")
            else:
...
_date_tag = f"<span style='color:#666; font-size:11px; font-weight:normal;'>（美股 {_macro_date} 收盤）</span>" if _macro_date else ""
...
with st.expander("🤖 系統自主選股模擬倉（做多 vs 做空 勝率PK）", expanded=False):
    ...
    if _stats['holding']:
        ...
        st.caption("🔴=做多／🔵=做空。選股理由記錄了每檔當初為什麼被系統選中，"
