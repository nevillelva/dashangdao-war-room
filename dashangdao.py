...
st.set_page_config(layout="wide", page_title="54088 - 終極大腦 V15.2")
...
st.markdown(f"<div style='text-align:right; color:#888; font-size:12px; margin-bottom:10px;'>大盤天候：<strong style='color:{weather_color};'>{weather_str}</strong> | V15.2 終極完全版 | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</div>", unsafe_allow_html=True)
...
def run_parallel_scan(target_codes, mode):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for i, future in enumerate(concurrent.futures.as_completed(future_to_code)):
            d = future.result()
            if d:
                # 戰術解放：移除對 "⏳" 的排斥，只過濾絕對危險的 "❌"，讓強勢但乖離稍大的股票也能顯示，供總指揮納入觀測雷達。
