...
# 必須在第一行執行
st.set_page_config(layout="wide", page_title="54088 - 終極大腦 V16.3", initial_sidebar_state="expanded")

# ==========================================
# 🛡️ 步驟一：狀態記憶體強制初始化 (杜絕所有 NameError)
# ==========================================
...
def cb_ui_sync(): 
    ...
    st.cache_data.clear() # 強制清除快取，獲取最新報價
...
def cb_buy_stock(code, raw_data, cat, ui_key_prefix):
    ...
    st.session_state.portfolio[code] = {
        ...
        "opt_event_vanish": False, "opt_earnings_miss": False, "opt_leader_crash": False, "opt_margin_call": False
    }
    ...
    for ext in [".TW", ".TWO"]:
        try:
            ...
            if not hist.empty and len(hist) > 15:
                try:
                    tick = tk.history(period="1d", interval="1m").dropna(subset=['Close'])
                    cp = float(tick['Close'].iloc[-1]) if not tick.empty else float(hist['Close'].iloc[-1])
                except: cp = float(hist['Close'].iloc[-1])
                
                try: prev_p = float(tk.fast_info.previous_close)
                except: prev_p = max(float(hist['Close'].iloc[-2]), 0.001)
                
                return hist, cp, prev_p
        except: continue
...
def calculate_tactical_signals(symbol_data, category_type="main", mode="短線技術動能單", manual_target=0.0, portfolio_data=None, manual_prices_dict=None, is_macro_panic_global=False):
    try:
        ...
        hist, current_price, prev_price = get_stock_history_cached(symbol)
        
        if hist.empty or current_price <= 0:
            return {
                "name": stock_name, "code": symbol, "price": 0.0, "gain": 0.0, "cost": 0.0, "cost_label": "網路中斷", "buy_zone": "0-0",
                "shd": "?", "chip_code": chip_code, "chip": "⚖️", "val_code": val_code, "val": "⚪",
                "kdj": "⚠️ 指標缺失", "chip_desc": "無資料", "val_desc": "無資料", "kdj_desc": "連線異常", 
                "downgrade_alert": "🚨 系統保護模式啟動", "signal": "❌ 【報價連線異常】系統暫停監視！", 
                "color": "#888", "signal_bg": "#111", "ai_tags": ["⚠️ 連線異常"], "exit_s": "未知", "exit_price": "0", "exit_color": "#888", "exit_bg": "#333", 
                ...
                "tactical_summary": "📡 系統目前無法連線取得報價，建議暫時在旁觀望。"
            }
        ...
        # 容錯處理：fillna 避免 NaN
        hist['Close'] = hist['Close'].fillna(method='bfill')
        ...
        if kdj_golden_cross: kdj_signal, kdj_desc = "📈 低檔金叉", "準備反彈"
        elif k > 80 and current_price < open_p: kdj_signal, kdj_desc = "💀 利多出盡", "高檔倒貨"
        elif k > 70 and k < d_val: kdj_signal, kdj_desc = "📉 高檔死叉", "動能衰退"
        else: kdj_signal, kdj_desc = "〰️ 震盪盤整", "無特殊訊號"

        entry_price = float(portfolio_data['entry_price']) if portfolio_data else 0.0
        ...
        tactical_summary = "目前走勢進入混沌期，多空交戰中，建議在旁耐心觀望，保留現金。"

        if is_fake_breakout: 
            signal_text, color_border, signal_bg = f"{ACTION_NO} 🚨 假突破誘多警告！主力正在高檔出貨！", "#e74c3c", "#3a1515"
            is_shield_active = False; is_action_needed = True
            tactical_summary = "❌ 系統偵測到異常的假突破！上方套牢賣壓極重，這是主力慣用的誘多手法，嚴禁追高！"
        elif opt_event_vanish or opt_earnings_miss or opt_leader_crash: 
            signal_text, color_border, signal_bg = f"{ACTION_NO} 買進理由已消失，請立刻撤退。", "#e74c3c", "#3a1515"
            is_shield_active = False; is_action_needed = True
            tactical_summary = "❌ 已觸發您設定的霸王紀律底線！請立刻無腦市價砍單，絕對不可心存僥倖凹單！"
        elif entry_price > 0 and roi_pct <= -10.0: 
            signal_text, color_border, signal_bg = f"{ACTION_NO} 觸發 10% 絕對停損結界！", "#e74c3c", "#3a1515"
            is_shield_active = False; is_action_needed = True
            tactical_summary = "❌ 虧損已達 10% 絕對紀律界線！請立刻市價全數平倉，留得青山在，保護核心本金！"
        elif sell_cond_count >= 2 and roi_pct > 0: 
            signal_text, color_border, signal_bg = f"{ACTION_HOLD} 觸發賣出三要件，請分批停利。", "#f1c40f", "#3a3015"
            is_action_needed = True
            tactical_summary = "🟡 雖然帳面仍在獲利，但技術面已現敗象，請嚴格執行分批停利，將現金真實入袋。"
        elif sell_cond_count >= 2 and roi_pct <= 0: 
            signal_text, color_border, signal_bg = f"{ACTION_NO} 觸發賣出三要件，短線已經轉空。", "#e74c3c", "#3a1515"
            is_action_needed = True
            tactical_summary = "❌ 股價破線且爆出大量，短線趨勢已死，不要再期待反彈，直接認賠殺出換股操作。"
        elif is_macro_panic_global: 
            if current_price <= buy_high: 
                signal_text, color_border, signal_bg = f"{ACTION_YES} 斷頭潮來臨！左側長線準備重壓！", "#00FF00", "#153a20"; is_golden = True
                tactical_summary = "✅ 趁大盤恐慌下殺，此標的已殺入極度便宜的超跌區，浮現超額價值，適合勇敢左側買進！"
            else: 
                signal_text, color_border, signal_bg = f"{ACTION_WAIT} 等待恐慌斷頭賣壓打下來再撿！", "#f39c12", "#3a3015"
                tactical_summary = "⏳ 股價尚未殺入足夠便宜的安全區，請耐心等待市場恐慌情緒發酵。"
        elif is_first_red:
            signal_text, color_border, signal_bg = f"{ACTION_YES} ✨ 破繭第一根！底部爆量強勢起漲！", "#00FF00", "#153a20"; is_golden = True
            tactical_summary = "✅ 出現完美的右側第一根起漲點！動能極強且風險可控，請嚴守停損紀律，大膽切入試單。"
        else:
            if is_shield_active and gain < -2.0: 
                signal_text, color_border, signal_bg = f"🛡️ 【長線護盾】 系統已過濾假跌破，安心抱單！", "#3498db", "#152a3a"
                tactical_summary = "🛡️ 長線波段距離目標價尚遠，系統已幫您自動屏蔽短線的下殺雜音，請安心抱牢。"
            elif val_code == "3": 
                signal_text, color_border, signal_bg = f"{ACTION_NO} 股價估值太貴已撞天花板，絕對別買！", "#e74c3c", "#3a1515"
                tactical_summary = "❌ 系統精算目前價格嚴重偏高，買進的期望值極低，請完全避開此標的！"
            elif (is_ma_bullish or buy_cond_count >= 1) and current_price > (buy_high * 1.05):
                signal_text, color_border, signal_bg = f"{ACTION_WAIT} 偏離防守區太遠，請等量縮拉回再買！", "#f39c12", "#3a3015"
                tactical_summary = "⏳ 雖然整體動能向上，但現在追高風險過大，停損會拉得很遠，請等量縮拉回均線再佈局。"
                ...
                tactical_summary = "✅ 攻擊動能已經點火，完全符合右側進場標準！設定好停損線後請果斷切入。"
            else: 
                signal_text, color_border, signal_bg = f"{ACTION_HOLD} 股價卡在區間震盪，在旁邊輕鬆看戲。", "#ccc", "#2b2b36"
        ...
        spotter_html = f"<div class='my-tooltip' style='background:#3a1515; padding:10px; border-radius:8px; margin-bottom:12px; border-left: 5px solid #e74c3c; display:block; width:100%;'><div style='font-size:12px; color:#ddd;'>🚨 撤退雷達：<strong style='color:#e74c3c; font-size:14px;'>🔴 危險訊號準備賣出</strong></div></div>" if sell_cond_count >= 2 else ""
        ...
        return {
            ...
            "signal_bg": signal_bg, "ai_tags": ai_tags, "extra_badge": "💰 高息防禦" if fund_info.get('Yield',0)>=5 else "", 
            ...
            "spotter_html": spotter_html, "buy_html": buy_html, "jail_html": "", "is_overridden": is_overridden, 
...
with st.sidebar:
    ...
    def run_parallel_scan(target_codes, mode, current_panic_state):
        results = []
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
            future_to_code = {executor.submit(calculate_tactical_signals, f"{c}:?:?:?:?", "scan", "短線技術動能單", 0.0, None, {}, current_panic_state): c for c in target_codes}
            for future in concurrent.futures.as_completed(future_to_code):
                try: d = future.result()
                except: d = None
                if d and "❌" not in d['signal'] and d['price'] > 0:
                    if mode == "golden" and d['is_golden']: results.append(d)
                    elif mode == "first_red" and "第一根" in "".join(d['ai_tags']): results.append(d)
        return results

    if st.button("🚀 黃金起漲與魚身", use_container_width=True):
        with st.spinner("向全市場發射掃描波..."):
            st.session_state.scan_results = run_parallel_scan(FULL_MARKET_CODES, "golden", is_panic)
            st.session_state.scan_mode = "golden"; st.rerun()
    if st.button("✨ 破繭第一根專區", use_container_width=True):
        with st.spinner("過濾剛爆量突破之標的..."):
            st.session_state.scan_results = run_parallel_scan(FULL_MARKET_CODES, "first_red", is_panic)
...
with col_title: st.markdown("<h1 style='color:#FFB300; margin: 0;'>54088 戰情室</h1>", unsafe_allow_html=True)
with col_sync:
...
for code, p in st.session_state.portfolio.items(): port_loaded[code] = calculate_tactical_signals(p['raw_data'], p['cat'], p.get('mode', '短線技術動能單'), p.get('manual_target', 0.0), p, current_manual_prices, is_panic)
for code, p in st.session_state.pinned_stocks.items(): pin_loaded[code] = calculate_tactical_signals(p['raw_data'], p['cat'], manual_prices_dict=current_manual_prices, is_macro_panic_global=is_panic)
...
market_suggestion = "🩸 【斷頭潮啟動】大盤恐慌崩跌！切換「左側價值」重壓便宜股！" if is_panic else ("💡 大盤多頭健康 ➡️ 適合【🚀 右側動能狙擊】" if is_bull_market else "💡 大盤恐慌震盪 ➡️ 適合【🛡️ 左側防禦佈局】")
...
<div class='hud-metric'><span style='color:#aaa;'>庫存 / 雷達</span> <strong style='color:#fff;'>{len(port_loaded)} / {len(pin_loaded)} 檔</strong></div>
...
if st.session_state.sentinel_active:
    ...
    if action_needed > 0 or golden_targets > 0:
        ...
        components.html(f"""<script>
            var audio = new Audio('https://actions.google.com/sounds/v1/alarms/beep_short.ogg'); audio.play().catch(e=>{{}});
            if (Notification.permission === 'granted') {{ new Notification('🚨 54088 警報', {{body: '{alert_msg}'}}); }}
            else if (Notification.permission !== 'denied') {{ Notification.requestPermission().then(p => {{ if (p==='granted') new Notification('🚨', {{body: '{alert_msg}'}}); }}); }}
        </script>""", height=0)
...
if search_query:
    ...
    if clean_code:
        ...
        if d:
            ...
            <div style="font-size:32px; font-weight:bold; margin-bottom: 10px; display:flex; gap:12px;">{d['price']:.2f} <span style="font-size:16px; color:{'#ff4d4d' if d['gain']>0 else '#00FF00'}; background-color:{'#3a1515' if d['gain']>0 else '#153a20'}; padding:4px 10px; border-radius:6px;">{d['gain']:+.1f}%</span></div>
            ...
            <div class="{summary_class}">📝 戰術小結：{d['tactical_summary']}</div></div>""", unsafe_allow_html=True)
            if d['code'] not in st.session_state.pinned_stocks and d['code'] not in st.session_state.portfolio:
                st.button(f"📌 加入觀測雷達", key=f"pin_search_{d['code']}", use_container_width=True, on_click=cb_pin_stock, args=(d['code'], f"{d['code']}:?:0:?:?:0", 'search'))
...
if st.session_state.temp_intel:
    ...
    for i, item in enumerate(st.session_state.temp_intel):
        ...
        if d:
            with cols[i % 2]:
                st.markdown(f"""<div style="border: 2px solid {d['color']}; border-radius: 8px; padding: 15px; background-color: #16191f; margin-bottom: 5px;"><div style="font-weight:bold; font-size:18px;">{d['name']} ({d['code']}) <span style="font-size:14px; color:{'#ff4d4d' if d['gain']>0 else '#00FF00'};">{d['gain']:+.1f}%</span></div><div class="{'tactical-danger' if d.get('is_action_needed') else 'tactical-summary'}" style="margin-top:5px; padding:5px;">{d['tactical_summary']}</div></div>""", unsafe_allow_html=True)
                st.button(f"📌 加入觀測雷達", key=f"pin_temp_{d['code']}", use_container_width=True, on_click=cb_pin_stock, args=(d['code'], item['raw_data'], 'temp'))
...
if st.session_state.pinned_stocks:
    ...
    for i, (code, p_data) in enumerate(list(st.session_state.pinned_stocks.items())):
        if code not in st.session_state.portfolio:
            ...
            if d:
                with cols[i % 2]:
                    st.markdown(f"""<div style="border: 1px solid {d['color']}; border-radius: 8px; padding: 15px; background-color: #16191f; margin-bottom: 5px;"><div style="font-weight:bold; font-size:18px;">{d['name']} ({d['code']}) <span style="font-size:14px; color:{'#ff4d4d' if d['gain']>0 else '#00FF00'};">{d['gain']:+.1f}%</span></div><div class="{'tactical-danger' if d.get('is_action_needed') else 'tactical-summary'}" style="margin-top:5px;">{d['tactical_summary']}</div></div>""", unsafe_allow_html=True)
                    c1, c2 = st.columns(2)
                    c1.button(f"⚡ 買進", key=f"buy_pin_{code}", use_container_width=True, on_click=cb_buy_stock, args=(code, p_data['raw_data'], p_data['cat'], "pin"))
...
if scan_mode_current := st.session_state.get('scan_mode', ""):
    ...
    if not st.session_state.scan_results:
        st.warning("⚠️ 雷達掃描完畢，未發現符合極限條件的標的！系統已過濾危險名單與死魚股。")
    else:
        cols = st.columns(2)
        for i, d in enumerate([x for x in st.session_state.scan_results if x['code'] not in st.session_state.portfolio and x['code'] not in st.session_state.pinned_stocks]):
            with cols[i % 2]:
                st.markdown(f"""<div style="border: 1px solid {d['color']}; border-radius: 8px; padding: 15px; background-color: #16191f; margin-bottom: 5px;"><div style="font-weight:bold; font-size:18px;">{d['name']} ({d['code']}) <span style="font-size:14px; color:{'#ff4d4d' if d['gain']>0 else '#00FF00'};">{d['gain']:+.1f}%</span></div><div class="tactical-summary" style="margin-top:5px;">{d['tactical_summary']}</div></div>""", unsafe_allow_html=True)
                st.button(f"📌 鎖定觀測", key=f"pin_scan_{d['code']}", use_container_width=True, on_click=cb_pin_stock, args=(d['code'], f"{d['code']}:?:?:?:?:?", "scan"))
