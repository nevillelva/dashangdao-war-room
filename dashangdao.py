...
st.set_page_config(layout="wide", page_title="54088 - V13")
...
def calculate_tactical_signals(symbol_data, category_type="main", mode="短線技術動能單", manual_target=0.0, portfolio_data=None):
    try:
        ...
        hist = pd.DataFrame()
        ticker = None
        # 尋找上市歷史資料 (將 1y 升級為 2y，徹底解決年線 nan 盲區)
        try:
            temp_ticker = yf.Ticker(f"{symbol}.TW")
            temp_hist = temp_ticker.history(period="2y") # 修正點：1y -> 2y
        ...
        if opt_event_vanish: signal_text, color_border, signal_bg = f"{ACTION_NO} 買進核心理由消失，霸王逃命條款觸發，強制撤退。", "#e74c3c", "#3a1515"; is_shield_active = False
            ...
            elif (buy_cond_count >= 1 or is_ma_bullish or w_bottom_breakout) and not f_fail: signal_text, color_border, signal_bg = f"{ACTION_YES} 突破或均線多頭確立！(右側極速狙擊)", "#00FF00", "#153a20"
...
st.markdown(f"<div style='text-align:right; color:#888; font-size:12px; margin-bottom:10px;'>大盤天候：<strong style='color:{weather_color};'>{weather_str}</strong> | V13 終極全自動大腦 | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</div>", unsafe_allow_html=True)
