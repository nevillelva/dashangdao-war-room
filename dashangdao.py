...
def evaluate_overnight_gate(macro):
    ...
    danger = []
    for key in ('那斯達克', '標普500', '費城半導體', '那斯達克期貨', '標普期貨', '台指期'):
        d = macro.get(key, {})
...
_macro_chips = []
for _name in ('那斯達克', '標普500', '費城半導體', '那斯達克期貨', '標普期貨', '台積電ADR', '聯電ADR', '台指期', '美元台幣'):
    _d = _macro.get(_name, {})
    if _d.get('ok'):
...
with st.expander("🤖 系統自主選股模擬倉（做多 vs 做空 勝率PK）", expanded=False):
    ...
    if st.button("▶️ 立即執行一次系統選股", key="run_sys_pick", use_container_width=True):
        ...
        if not _pool:
            ...
            with st.spinner(f"全市場掃描 {len(_pool)} 檔，挑選多空候選中（檔數越多耗時越久）..."):
