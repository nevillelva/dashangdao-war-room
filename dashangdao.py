...
st.set_page_config(layout="wide", page_title="54088 - 終極大腦 V16.2", initial_sidebar_state="expanded")
...
CHIP_MAP = {"1": "🐳 大戶進駐", "2": "🩸 外資提款", "0": "⚖️ 籌碼平穩", "?": "❓ 籌碼待查"}
...
# ==========================================
# 🚨 【核心修復點】全域變數頂層化 (Global Context Hoisting)
# 確保在任何 UI 按鈕按下前，系統已備妥所有大盤數據，徹底根絕 NameError 崩潰
# ==========================================
    ...
    try:
        ...
        if hist.empty or ticker is None:
            ...
            return {
                ...
                "downgrade_alert": "🚨 API 阻擋，切換手動模式", "signal": "❌ 【報價伺服器無回應】請手動輸入現價！", 
        ...
        if is_fake_breakout: 
            ...
            is_shield_active = False; is_action_needed = True
            tactical_summary = "❌ 已觸發霸王紀律底線！立刻無腦市價砍單，絕對不可凹單留戀！"
        elif kdj_danger_trap: 
            signal_text, color_border, signal_bg = f"{ACTION_NO} 高檔割韭菜訊號，無條件快逃！", "#e74c3c", "#3a1515"
            is_shield_active = False; is_action_needed = True
            tactical_summary = "❌ KD極度高檔且爆量收黑，主力出貨意圖明確，隨時可能大跌！"
        elif entry_price > 0 and roi_pct <= -10.0: 
            signal_text, color_border, signal_bg = f"{ACTION_NO} 觸發 10% 絕對停損！", "#e74c3c", "#3a1515"
            is_shield_active = False; is_action_needed = True
            tactical_summary = "❌ 虧損已達 10% 絕對紀律！請立刻市價全數平倉，保護核心本金！"
        elif sell_cond_count >= 2 and roi_pct > 0: 
            signal_text, color_border, signal_bg = f"{ACTION_HOLD} 觸發賣出三要件，分批停利。", "#f1c40f", "#3a3015"
            is_action_needed = True
            tactical_summary = "🟡 帳面仍在獲利中，但技術面已現敗象，請嚴格執行分批停利。"
        elif sell_cond_count >= 2 and roi_pct <= 0: 
            signal_text, color_border, signal_bg = f"{ACTION_NO} 觸發賣出三要件，短線已經轉空。", "#e74c3c", "#3a1515"
            is_action_needed = True
            tactical_summary = "❌ 股價破線且爆大量，短線趨勢已死，直接認賠殺出。"
        elif is_macro_panic: 
            if current_price <= buy_high: 
                signal_text, color_border, signal_bg = f"{ACTION_YES} 斷頭潮來臨！長線準備重壓！", "#00FF00", "#153a20"; is_golden = True
                tactical_summary = "✅ 趁大盤恐慌斷頭，標的已超跌浮現超額價值，適合分批左側買進！"
            else: 
                signal_text, color_border, signal_bg = f"{ACTION_WAIT} 等斷頭賣壓打下來再撿！", "#f39c12", "#3a3015"
                tactical_summary = "⏳ 尚未殺入足夠便宜的安全區，耐心等待恐慌發酵。"
        elif is_first_red:
            signal_text, color_border, signal_bg = f"{ACTION_YES} ✨ 破繭第一根！底部爆量起漲！", "#00FF00", "#153a20"; is_golden = True
            tactical_summary = "✅ 完美的右側第一根起漲點！動能極強，請嚴守 10% 停損切入試單。"
        elif is_break_ma5 and weekly_k_bullish: 
            signal_text, color_border, signal_bg = f"{ACTION_HOLD} 破 5MA，但週 K 線保護中。", "#f39c12", "#3a2515"
            tactical_summary = "🛡️ 雖然短線跌破，但長線趨勢向上，主力洗盤機率高，暫不動作。"
        else:
            if is_shield_active and gain < -2.0: 
                signal_text, color_border, signal_bg = f"🛡️ 【長線護盾】 過濾假跌破，安心抱單！", "#3498db", "#152a3a"
                tactical_summary = "🛡️ 長線波段距離目標價尚遠，系統已屏蔽短線雜音，安心抱牢。"
            elif val_code == "3": 
                signal_text, color_border, signal_bg = f"{ACTION_NO} 太貴已撞天花板，千萬別買！", "#e74c3c", "#3a1515"
                tactical_summary = "❌ 系統估算價格嚴重偏高，買進期望值極低，請避開！"
            elif mode == "長線價值波段單" and manual_target > 0 and ((manual_target - current_price) / max(current_price, 0.001) * 100) < 30.0:
                signal_text, color_border, signal_bg = f"{ACTION_WAIT} 潛在報酬小於 30%，不值得！", "#f39c12", "#3a3015"
                tactical_summary = "⏳ 距離您的目標價獲利空間太小，風險大於報酬，等待更佳買點。"
            elif (is_ma_bullish or buy_cond_count >= 1) and current_price > (buy_high * 1.03):
                signal_text, color_border, signal_bg = f"{ACTION_WAIT} 偏離防守區，請等拉回再買！", "#f39c12", "#3a3015"
                tactical_summary = "⏳ 動能雖在但追高風險過大，停損會拉很遠，請等量縮拉回再佈局。"
            elif is_cold_to_hot:
                signal_text, color_border, signal_bg = f"{ACTION_YES} 🌟 冷門轉機股甦醒！具備妖股潛力！", "#e056fd", "#2c153a"; is_golden = True
                tactical_summary = "✅ 長期無人問津突然爆出異常攻擊量，代表無聲吃貨完畢，準備發動大行情！"
                ...
                tactical_summary = "✅ 攻擊動能點火，符合右側進場標準！設定好 10% 停損線後果斷切入。"
            elif is_fish_head and not f_fail: 
                signal_text, color_border, signal_bg = f"{ACTION_WAIT} 發現魚頭特徵！爆量打底中。", "#3498db", "#152a3a"
                tactical_summary = "⏳ 處於安全的魚頭打底期，可小部位資金試單，等待爆量表態後加碼。"
            elif f_fail and (buy_cond_count >= 1 or is_fish_head): 
                signal_text, color_border, signal_bg = f"{ACTION_NO} 財報防彈網未過！拒絕放行。", "#e74c3c", "#3a1515"
                tactical_summary = "❌ 雖然線型不錯，但基本面財報數據不合格，系統拒絕發布波段買進許可。"
            else: 
                signal_text, color_border, signal_bg = f"{ACTION_HOLD} 卡在區間震盪，在旁邊看戲。", "#ccc", "#2b2b36"
        ...
        if val_code == "3": exit_s, exit_p, exit_c, exit_bg = "🔴 估值偏高強制定利", f"{current_price:.1f}", "#e74c3c", "#3a1515"
        elif roi_pct > 15.0: exit_s, exit_p, exit_c, exit_bg = "🛡️ 破五日線停利", f"{ma5:.1f}", "#e67e22", "#3a2515"
...
with st.sidebar:
    ...
    if st.button("🛡️ 總經防禦高息池", use_container_width=True):
        with st.spinner("向全市場發射掃描波..."):
            st.session_state.scan_results = run_parallel_scan([c for c, data in FUNDAMENTAL_DB.items() if data['Yield'] >= 5.0 or data['PE'] < 12.0], "yield", is_panic)
...
with col_sync:
    ...
    st.button("🔄 同步", use_container_width=True, on_click=cb_ui_sync) 
    st.markdown("</div>", unsafe_allow_html=True)
with col_logout:
