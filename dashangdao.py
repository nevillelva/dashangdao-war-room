...
def init_session_state():
    ...
    if not hasattr(st.session_state, 'bigholder_override'): st.session_state.bigholder_override = {} 
        ...
        if os.path.exists(INST_HISTORY_FILE):
            try:
                with open(INST_HISTORY_FILE, "r", encoding="utf-8") as f:
                    ...
                    invalid_dates = []
                    for d, stocks in st.session_state.inst_history.items():
                        try:
                            date_obj = datetime.strptime(d, "%Y-%m-%d")
                            if date_obj.weekday() >= 5: 
                                all_zero = True
                                for code, data in stocks.items():
                                    if data.get('foreign', 0) != 0 or data.get('trust', 0) != 0 or data.get('dealer', 0) != 0:
                                        all_zero = False
                                        break
                                if all_zero: invalid_dates.append(d)
                        except: pass
                    for d in invalid_dates:
                        del st.session_state.inst_history[d]
...
def calculate_comprehensive_signals(symbol, enable_doomsday=False):
    ...
    f_single = t_single = d_single = margin_diff = big_holder = 0.0
    ...
    if sorted_dates:
        ...
        f_single, t_single, d_single, margin_diff = latest_data.get('foreign', 0), latest_data.get('trust', 0), latest_data.get('dealer', 0), latest_data.get('margin', 0)
        
        for d in sorted_dates:
            day_stock_data = st.session_state.inst_history[d].get(symbol, {})
            bh_val = day_stock_data.get('big_holder', 0.0)
            
            if isinstance(bh_val, (int, float)) and bh_val > 0.0:
                big_holder = bh_val
                big_holder_date = day_stock_data.get('big_holder_date', '')
                if not big_holder_date:
                    try: big_holder_date = datetime.strptime(d, "%Y-%m-%d").strftime("%m/%d")
                    except: big_holder_date = d
                break
            elif isinstance(bh_val, str) and "[" in bh_val:
                if big_holder == 0.0: big_holder = bh_val
                
        if big_holder == 0.0:
            big_holder = latest_data.get('big_holder', 0.0)
            big_holder_date = latest_data.get('big_holder_date', '')
    ...
    return {
        ...
        "big_holder_date": big_holder_date, 
    ...
    for file_bytes in uploaded_files:
        ...
        try:
            ...
            if date_match:
                ...
                file_date = get_last_trading_date()  
            ...
            for index, row in df.iterrows():
                ...
                if len(code) == 4 and code.isdigit():
                    ...
                    payload = {'foreign': f_buy, 'trust': t_buy, 'dealer': (d1 + d2), 'margin': existing.get('margin', 0), 'big_holder': existing.get('big_holder', 0.0), 'big_holder_date': existing.get('big_holder_date', '')}
...
def execute_heavy_data_sync(target_codes, target_date):
    ...
    for c in target_codes:
        existing_data = history_db.get(actual_target_date, {}).get(c, {})
        if not existing_data or existing_data.get('big_holder', 0.0) == 0.0 or isinstance(existing_data.get('big_holder'), str):
            missing.append(c)
...
def sync_single_stock_finmind(code):
    ...
    try:
        ...
        if inst_success or margin_success or bh_success:
            ...
            # V150 新增：API成功連線，但尚無今日最新資料，安全沿用歷史備份
            if target_date not in st.session_state.inst_history:
                st.session_state.inst_history[target_date] = {}
            st.session_state.inst_history[target_date][code] = base_payload
            save_local_db_isolated()
            ...
            fetch_finmind_revenue.clear() 
            return True, "API連線成功，但尚無今日資料，自動沿用歷史備份"
        else:
            fetch_finmind_revenue.clear()
...
with st.sidebar:
    ...
    with st.expander("📖 統籌戰術解密說明書", expanded=False):
        # V150 新增：強制 UI 亮白顯示並寫入具體戰略邏輯
        st.markdown("""
        <div style="font-size:13px; line-height:1.8; color:#ffffff; background:#1e1e24; padding:15px; border-radius:8px; border:1px solid #444;">
        <b style='color:#f1c40f; font-size:14px;'>🛡️ V150 戰情室濾網邏輯大公開</b><br>
        <b style='color:#00d2ff;'>查1.主升段突擊</b>: 首根長紅 + 爆量比>=2.0 + KDJ金叉<br>
        <b style='color:#00d2ff;'>查2.魚頭慢伏支撐</b>: 股價站上季線(60MA) + 爆量比>=1.2<br>
        <b style='color:#00d2ff;'>查3.價值投資與循環</b>: 綜合評分>=60 + 無地雷標記<br>
        <b style='color:#00d2ff;'>查4.投信作帳集團股</b>: 投信單日買超大於0張<br>
        <b style='color:#00d2ff;'>查5.籌碼外資霸王色</b>: 外資單日買超 + 融資餘額減少(沉澱)<br>
        <b style='color:#00d2ff;'>查6.營收雙增爆發突破</b>: 營收 YoY 年增 > 20%<br>
        <b style='color:#00d2ff;'>查8.昨日強勢動能延續</b>: 昨日漲幅強勢 (>5%)<br>
        <b style='color:#00d2ff;'>查9.均線糾結爆量突破</b>: 今日爆量比 >= 2.0x<br>
        <b style='color:#00d2ff;'>查10.籌碼沉澱量縮潛伏</b>: 今日量縮 > 40% + 融資減少<br>
        <b style='color:#00d2ff;'>查11.除權息尋寶雷達</b>: 現金殖利率 >= 設定門檻 (預設4.5%)<br>
        <b style='color:#00d2ff;'>查12.K線型態尋寶型</b>: 觸發特定K線型態 (如紅三兵、長紅吞噬)<br>
        <hr style='border-color:#444; margin:10px 0;'>
        <b style='color:#00d2ff;'>情報雷達</b>: 動態抓取注入的外部情資 (如股癌、法人報告)<br>
        <b style='color:#00d2ff;'>🏆 情報黃金交叉</b>: 該標的同時出現在 2 種以上的情報來源中
        </div>
...
# ==============================================================================
# 十、 主畫面：V150 物理情報大腦注入中樞
# ==============================================================================
st.title("🚀 54088 戰情室 V150.0 終極戰術武裝版")
...
st.markdown(f"""<div class='hud-box'>
    ...
    <div style='color:#ddd; font-size:14px;'><b>大盤氣象：</b> {weather_str} | <b>安全狀態：</b> V150.0 終極戰術武裝版</div>
...
def render_commander_stock_card(c, is_portfolio=False, profit=0, roi=0, ent_p=0):
    ...
    bh_date_label = f" ({c.get('big_holder_date')})" if c.get('big_holder_date') else ""
    
    bh_raw = c.get('big_holder', 0.0)
    bh_display = f"{bh_raw}%" if isinstance(bh_raw, (int, float)) else f"<span style='color:#e67e22; font-size:11px;'>{bh_raw}</span>"
    ...
    if vol_ratio > 1.5:
        ...
        vol_semantic = "<span style='color:#00d2ff; font-size:12px; margin-left:6px;'>[🧊 量縮洗盤沉澱]</span>"

    # V150 新增：字卡標題掛載 K線大局觀
    k_patterns = c.get('detected_patterns', [])
    k_tags = ""
    if k_patterns:
        k_text = k_patterns[0].get('text', '')
        k_icon = "📉" if "黑" in k_text or "高檔" in k_text else "🔥"
        k_tags = f"<span style='font-size:13px; background:#2c3e50; padding:3px 8px; border-radius:5px; color:#f1c40f; margin-left:12px; vertical-align: middle;'>{k_icon} {k_text}</span>"
    else:
        k_tags = f"<span style='font-size:13px; background:#2c3e50; padding:3px 8px; border-radius:5px; color:#aaaaaa; margin-left:12px; vertical-align: middle;'>⚖️ 壓縮盤整</span>"
...
<div style="background:#0e1117; padding:8px; border-radius:4px; margin-bottom:10px; color:#eeeeee;">
    ...
    <div style="font-size:13px; display:flex; justify-content:space-between;">
        <span><span class="m-tooltip">爆量比<span class="m-tooltiptext">今日量 ÷ 5日均量</span></span>: <strong style="color:#e67e22;">{float(c.get('vol_ratio',0)):.1f}x</strong>{vol_semantic}</span>
...
<div class="zone-box">
    <div class="shadow-box">
        ...
        <div style="font-size:12px; border-top:1px dashed #444; padding-top:6px; display:flex; justify-content:space-between; color:#aaa;">
            <span class="m-tooltip">千張大戶{bh_date_label}<span class="m-tooltiptext">大股東持股總比例 (支援跨假期無痛自動回溯防護)</span></span>: <strong style="color:#00d2ff;">{bh_display}</strong>
...
def render_action_buttons(card, code, is_portfolio):
    ...
    with st.expander("⚙️ 啟動資料校正與客觀數據補給", expanded=False):
        ...
        m_month = m_cols[0].text_input("月份 (如:06月)", value="06月", key=f"my_mo_{code}{btn_suffix}")
        m_y = m_cols[1].number_input("年增 (%)", -100.0, 1000.0, float(card.get('rev_yoy', 0.0)), 0.1, key=f"my_y_{code}{btn_suffix}")
        ...
        b_date = b_cols[0].text_input("結算日期 (如:07/10)", value=datetime.now().strftime("%m/%d"), key=f"my_bh_date_{code}{btn_suffix}")
        
        raw_b_val = card.get('big_holder', 0.0)
    ...
    with st.expander("📥 貼上外部網頁版情報與裁決 (三方會審區)", expanded=False):
        ...
        c1, c2 = st.columns(2)
        nv_val = c1.text_area("📝 NVIDIA (DeepSeek) 網頁版報告", height=120, key=f"nv_txt_{code}{btn_suffix}")
        gm_val = c2.text_area("📝 Gemini 分析報告", height=120, key=f"gm_txt_{code}{btn_suffix}")
        cl_val = st.text_area("👑 Claude 總裁決報告 (將存入 100 筆歷史紀錄)", height=120, key=f"cl_txt_{code}{btn_suffix}")

        bc1, bc2 = st.columns(2)
        if bc1.button("🚀 一鍵打包送交 Claude", key=f"pack_{code}{btn_suffix}", use_container_width=True):
            bh_prompt_date = f"({card.get('big_holder_date')})" if card.get('big_holder_date') else ""
            bh_mega_val = card.get('big_holder', 0)
            bh_mega_str = f"{bh_mega_val}%" if isinstance(bh_mega_val, (int, float)) else str(bh_mega_val)
...
* 籌碼面 (近5日)：外資 {card.get('f_5d')}張 | 投信 {card.get('t_5d')}張 | 千張大戶{bh_prompt_date} {bh_mega_str}
