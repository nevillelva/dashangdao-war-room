...
st.set_page_config(layout="wide", page_title="54088 - 終極大腦 V15.5")
...
# ==========================================
# 🧠 核心量化演算法 (V15.5 純淨防崩潰版)
# ==========================================
# ⚠️ 注意：此函數已徹底隔離 st.session_state，確保多執行緒下絕對不會崩潰
def calculate_tactical_signals(symbol_data, category_type="main", mode="短線技術動能單", manual_target=0.0, portfolio_data=None, manual_prices_dict=None, is_macro_panic_global=False):
    try:
        ...
        for attempt in range(3):
            ...
            time.sleep(random.uniform(0.5, 1.5)) # 指數退避隱蔽等待
...
st.markdown(f"<div style='text-align:right; color:#888; font-size:12px; margin-bottom:10px;'>大盤天候：<strong style='color:{weather_color};'>{weather_str}</strong> | V15.5 終極除錯純淨版 | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</div>", unsafe_allow_html=True)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
        ...
        for i, future in enumerate(concurrent.futures.as_completed(future_to_code)):
            ...
            if d:
                ...
                crash_blocks += 1 # ⚠️ 精準捕獲所有因為 Bug 或斷線而死掉的運算
