...
def get_active_fm_token():
    idx = getattr(st.session_state, 'active_key_index', 0) % max(1, len(FINMIND_TOKENS))
    ...
    if df is not None and not df.empty:
        ...
        for _, row in df[::-1].iterrows():
            ...
            try:
                ...
                with _LAST_GOOD_LOCK: _LAST_GOOD_REVENUE[symbol] = result
                return result
            except Exception: continue

    with _LAST_GOOD_LOCK: last_good = _LAST_GOOD_REVENUE.get(symbol)
    if last_good:
...
def fetch_big_holder_with_recursion(code, token, target_date, initial_lookback=20, max_lookback=180):
    ...
    err_msg = "[⛔ API次數耗盡]" if last_err == "rate_limited" else "[📭 無資料]"
...
def determine_signal(current_price, ma5, ma20, foreign_buy, vol_ratio, is_open_high_close_low, buffer_pct, gain=0.0, enable_doomsday=False):
    score = 0
    reasons = []
    ...
    if foreign_buy > 0: score += 1; reasons.append(f"外買{foreign_buy:,.0f}")
    elif foreign_buy < 0: score -= 1; reasons.append(f"外賣{abs(foreign_buy):,.0f}")
...
def calculate_signals_worker(symbol, config):
    ...
    prev_5_vol = hist['Volume'].iloc[-6:-1] if len(hist) >= 6 else hist['Volume'].iloc[:-1]
    ...
    if not inst_df.empty:
        ...
        f_10d, t_10d = float(inst_df['foreign_buy'].sum()), float(inst_df['trust_buy'].sum())
    ...
    return {
        ...
        "vol": vol_today, "vol_5d_mean": vol_5d_mean, "vol_change_str": vol_change_str, "vol_ratio": vol_ratio, "vol_ratio_label": vol_ratio_label,
        ...
        "f_buy": f_single, "t_buy": t_single, "d_buy": d_single, "margin_diff": margin_diff,
...
def render_stock_card_ui(c, is_portfolio=False, profit=0, roi=0, ent_p=0):
    ...
    tooltip_vol = "<span class='m-tooltiptext'>小於0.6為量縮沉澱，0.8~1.2為正常換手，大於1.5為爆量。</span>"
    tags_html = f"<div style='display:flex; flex-wrap:wrap; gap:6px; align-items:center; margin-top:5px;'><span class='m-tooltip' style='white-space:nowrap; display:inline-block; background:#2a2a2a; padding:2px 8px; border-radius:4px; font-size:12px; color:#e67e22;'>{c.get('vol_ratio_label')} [{vol_semantic}]{tooltip_vol}</span><span style='white-space:nowrap; display:inline-block; background:#2a2a2a; padding:2px 8px; border-radius:4px; font-size:12px; color:#00FF00;'>{c.get('intraday_str')}</span></div>"
    ...
    tooltip_rsi = "<span class='m-tooltiptext'>大於70超買，小於30超賣。30~70為中立整理。</span>"
    rsi_html = f"<span class='m-tooltip'>RSI(14): <strong style='color:#fff;'>{rsi_v:.1f}</strong> <span style='background:{rsi_color}; color:#fff; padding:2px 6px; border-radius:4px; font-weight:bold; font-size:11px;'>{rsi_txt}</span>{tooltip_rsi}</span>"
    
    tooltip_bias = "<span class='m-tooltiptext'>起漲醞釀期通常貼近均線(0%~2%)。大於+5%短線過熱，小於-5%超跌反彈。</span>"
    bias_html = f"<span class='m-tooltip'>乖離率(20): <strong style='color:{bias_color if bias_color else '#fff'};'>{bias_v:+.2f}%</strong>" + (f" <span style='background:{bias_color}; color:#fff; padding:2px 6px; border-radius:4px; font-weight:bold; font-size:11px;'>{bias_txt}</span>" if bias_txt else "") + f"{tooltip_bias}</span>"
    ...
    if db_date:
        ...
        tooltip_warn = "<span class='m-tooltiptext'>證交所未更新今日籌碼，此為系統尋獲之最新舊資料。</span>"
        warn_icon = "" if db_date == datetime.now().strftime("%Y-%m-%d") else f"<span class='m-tooltip'> ⚠️{tooltip_warn}</span>"
    else: display_date, warn_icon = " (尚無資料)", ""
    ...
    if '攻擊' in sig_t: sig_tip = "實戰：帶量突破均線糾結，動能強勁。"
    elif '防守' in sig_t or '警告' in sig_t or '轉弱' in sig_t: sig_tip = "實戰：可能高檔倒貨、爆量下殺或破線轉弱，嚴格控管資金。"
    else: sig_tip = "實戰：目前盤整或溫和換手，無明確單向動能。"
    tooltip_sig = f"<span class='m-tooltiptext'><b>[評分級距說明]</b><br>🔥 偏多攻擊 (>= 3分)<br>🟡 觀察偏多 (1~2分)<br>⚖️ 中立震盪 (0分)<br>⚠️ 轉弱謹慎 (-1~-2分)<br>🔵 偏空防守 (<=-3分)<hr style='margin:4px 0; border-color:#666;'>{sig_tip}</span>"
    ...
    html_lines = [
        ...
        f"""<div style="display:flex; align-items:center;"><span style="font-size:32px; font-weight:bold; color:#ffffff;">{float(c.get('price',0)):.2f}</span><span style="font-size:15px; color:{gain_c}; background:{gain_b}; padding:3px 8px; border-radius:4px; margin-left:10px; font-weight:bold;">{float(c.get('gain',0)):+.2f}%</span></div>""",
        ...
        f"""<div style="font-size:13px; margin-bottom:4px;">總量: {c.get('vol'):,.0f}張 ({c.get('vol_change_str')[0]})</div>""",
        tags_html,
        f"""</div>""",
        f"""<div class="zone-box"><div class="zone-title">❤️ 第一戰區：基本與財報面</div>""",
        f"""<div style="font-size:13px; margin-bottom:4px;">營收 年增 <strong style="color:#ffffff;">({c.get('rev_month')})</strong>: <strong style="color:{yoy_color};">{yoy_val:.1f}%</strong> | 月增: <strong style="color:{'#ff4d4d' if mom_val>0 else '#00FF00'};">{mom_val:.1f}%</strong></div>""",
        f"""<div style="font-size:13px;">除權息資訊: <strong style="color:#d200ff;">{c.get('div_display')} (殖利率: {float(c.get('div_yield',0)):.1f}%)</strong></div></div>""",
        f"""<div class="zone-box"><div class="zone-title">⚔️ 第二戰區：技術與新指標防線</div>""",
        ...
        f"""<span class='m-tooltip' style='color:#ff4d4d;'>進攻參考:<span class='m-tooltiptext'>現價加上1倍ATR作為短線滿足點參考</span></span> {c.get('atk_zone')} | <span class='m-tooltip' style='color:#00FF00;'>防守停損:<span class='m-tooltiptext'>MA5扣除0.5倍ATR波動緩衝，防隨機洗盤</span></span> {c.get('def_line')} (緩衝 {c.get('buffer_pct')}%, <span class='m-tooltip'>ATR={c.get('atr_val'):.2f}<span class='m-tooltiptext'>真實波動幅度，衡量近期日均震幅，作為防守線緩衝。</span></span>)</div></div>""",
        f"""<div class="zone-box"><div class="shadow-box"><div class="zone-title">📊 第三戰區：三大法人與主力籌碼</div>""",
        ...
        f"""<div style="font-size:13px; margin-bottom:6px; display:flex; flex-wrap:wrap; gap:6px;"><b>[投信]</b> 單日<span style="color:#f1c40f;">({display_date}{warn_icon})</span>: <strong style="color:#ff4d4d;">{int(c.get('t_buy',0)):+,}張 ({float(c.get('t_pct',0)):+.2f}%)</strong> | 5日: <strong>{int(c.get('t_5d',0)):+,}張 ({float(c.get('t_5d_pct',0)):+.2f}%)</strong> | 10日: <strong>{int(c.get('t_10d',0)):+,}張 ({float(c.get('t_10d_pct',0)):+.2f}%)</strong></div>""",
        f"""<div style="font-size:12px; border-top:1px dashed #444; padding-top:6px; display:flex; justify-content:space-between; color:#aaa;"><span>千張大戶({c.get('big_holder_date')}): <strong style="color:#00d2ff;">{bh_display}</strong></span><span>自營商: {int(c.get('d_buy',0)):+,}張</span></div></div></div>""",
        ...
        if inst_success:
            with DB_LOCK:
                SQLITE_CONN.execute('''
                    INSERT INTO inst_holding (date, symbol, foreign_buy, trust_buy, dealer_buy, margin, big_holder, big_holder_date)
                    VALUES (?, ?, ?, ?, ?,
                        COALESCE((SELECT margin FROM inst_holding WHERE date=? AND symbol=?), 0),
                        0.0, '')
                    ON CONFLICT(date, symbol) DO UPDATE SET 
                        foreign_buy=excluded.foreign_buy, 
                        trust_buy=excluded.trust_buy, 
                        dealer_buy=excluded.dealer_buy;
                ''', (target_date, code, base_payload['foreign'], base_payload['trust'], base_payload['dealer'], target_date, code))
                SQLITE_CONN.commit()
            
            if bh_success: msg = "同步完成 (籌碼+大戶)"
            elif bh_result is None or bh_result.get('error'): msg = "同步完成 (籌碼)，⏳大戶無資料"
            else: msg = "同步完成 (籌碼)，大戶寫入失敗"
            return True, msg
...
with st.sidebar:
    ...
    with st.expander("📖 統籌戰術解密說明書", expanded=False):
        ...
        <b style='color:#f1c40f;'>🛡️ V155 戰情室濾網大公開</b><br>
...
# 十一、 主畫面渲染與按鈕邏輯
# ==============================================================================
st.title("🚀 54088 戰情室 V155 終極神盾版")

st.markdown(f"""<div class='hud-box'>
    <div style='color:#f1c40f; font-size:16px; font-weight:bold; margin-bottom:4px;'>📊 大將軍智慧 HUD 總覽</div>
    <div style='color:#ddd; font-size:14px;'><b>大盤氣象：</b> <span style='color:{weather_color}; font-weight:bold;'>上市大盤 {weather_str}</span> | <b>安全狀態：</b> V155 神盾版</div>
