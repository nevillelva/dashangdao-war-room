...
def render_stock_card_ui(c, is_portfolio=False, profit=0, roi=0, ent_p=0):
    ...
    <div style='display:flex; flex-wrap:wrap; gap:6px; align-items:center; margin-top:5px;'>
        <span style='white-space:nowrap; display:inline-block; background:#2a2a2a; padding:2px 8px; border-radius:4px; font-size:12px; color:#e67e22;'>{c.get('vol_ratio_label')} [{vol_semantic}]</span>
...
with st.sidebar:
    ...
    with st.expander("📖 統籌戰術解密說明書", expanded=False):
        ...
        <b style='color:#f1c40f;'>🛡️ V153.2 戰情室濾網大公開</b><br>
...
# 十、 主畫面：UI 渲染與三方會審區塊
# ==============================================================================
st.title("🚀 54088 戰情室 V153.2 穩定版")

st.markdown(f"""<div class='hud-box'>
    <div style='color:#f1c40f; font-size:16px; font-weight:bold; margin-bottom:4px;'>📊 大將軍智慧 HUD 總覽</div>
    <div style='color:#ddd; font-size:14px;'><b>大盤氣象：</b> <span style='color:{weather_color}; font-weight:bold;'>上市大盤 {weather_str}</span> | <b>安全狀態：</b> V153.2 鋼鐵防線版</div>
...
def render_stock_card_ui(c, is_portfolio=False, profit=0, roi=0, ent_p=0):
    ...
    rsi_html = f"<span class='m-tooltip'>RSI(14): <strong style='color:#fff;'>{rsi_v:.1f}</strong> <span style='background:{rsi_color}; color:#fff; padding:2px 6px; border-radius:4px; font-weight:bold; font-size:11px;'>{rsi_txt}</span><span class='m-tooltiptext'>大於70超買，小於30超賣</span></span>"
    bias_html = f"乖離率(20): <strong style='color:{bias_color if bias_color else '#fff'};'>{bias_v:+.2f}%</strong>" + (f" <span style='background:{bias_color}; color:#fff; padding:2px 6px; border-radius:4px; font-weight:bold; font-size:11px;'>{bias_txt}</span>" if bias_txt else "")
    ...
    bh_val = c.get('big_holder', 0.0)
    bh_display = f"{bh_val}%" if isinstance(bh_val, (int, float)) and bh_val > 0 else "📭 無資料"
