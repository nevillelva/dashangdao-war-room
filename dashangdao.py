...
def calculate_tactical_signals(symbol_data, category_type="main", mode="短線技術動能單", manual_target=0.0, portfolio_data=None):
    try:
        ...
        for attempt in range(3):
            ...
            time.sleep(random.uniform(0.5, 1.5)) # 隱蔽等待防封鎖
        ...
        is_cold_to_hot = (vol_5d <= 2.0) and is_attack_vol
...
st.markdown(f"<div style='text-align:right; color:#888; font-size:12px; margin-bottom:10px;'>大盤天候：<strong style='color:{weather_color};'>{weather_str}</strong> | V15 終極完美融合版 | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</div>", unsafe_allow_html=True)
...
def run_parallel_scan(target_codes, mode):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        future_to_code = {executor.submit(calculate_tactical_signals, f"{c}:?:?:?:?", "scan"): c for c in target_codes}
        progress_bar = st.progress(0)
        status_text = st.empty()
        for i, future in enumerate(concurrent.futures.as_completed(future_to_code)):
            d = future.result()
            if d:
                if mode == "golden" and (d['is_ma_bullish'] or d['w_bottom_breakout']) and d['buy_cond_count'] >= 1 and -5.0 <= d['diff_from_cost'] <= 10.0 and d['gain'] > -2.0 and not any(x in d['signal'] for x in ["❌", "⏳"]):
                    results.append(d)
                elif mode == "stealth" and d['vol_ratio'] >= 2.0 and -5.0 <= d['diff_from_cost'] <= 5.0 and not "❌" in d['signal']:
                    results.append(d)
                elif mode == "yield" and d['sell_cond_count'] < 2 and d['diff_from_cost'] >= -6.0 and (d['vol'] > 300) and (d['is_high_yield'] or d['is_cyclical']) and not any(x in d['signal'] for x in ["❌", "⏳"]):
                    results.append(d)
            progress_bar.progress(min((i + 1) / len(target_codes), 1.0))
            status_text.markdown(f"📡 多執行緒防封鎖群發掃描中 ({i+1}/{len(target_codes)})...")
    status_text.empty()
    progress_bar.empty()
    return results
