...
# ==========================================
st.set_page_config(layout="wide", page_title="54088 - 終極大腦 V16.5", initial_sidebar_state="expanded")

# 強制初始化所有 Session State，杜絕任何 NameError 或 KeyError
...
def cb_ui_sync(): 
    ...
    st.cache_data.clear() # 強制清除快取，獲取最新鮮資料
...
if not FULL_MARKET_CODES: FULL_MARKET_CODES = list(TW_STOCKS.keys())
    ...
    try:
        ...
        if hist.empty or current_price <= 0:
            return {
                ...
                "kdj": "⚠️ 無法判斷", "chip_desc": "無資料", "val_desc": "無資料", "kdj_desc": "連線異常", 
                ...
                "tactical_summary": "📡 系統目前無法連線取得報價，建議暫時在旁觀望，或檢查網路狀態。", "is_high_yield": False, "is_cyclical": False, "vol_ratio": 0.0, "diff_from_cost": 0.0, "vol": 0
            }
        ...
        return {
            ...
            "buy_status": buy_status, "buy_color": buy_color, "buy_bg": buy_bg, "is_overridden": is_overridden, 
...
for code, d in pin_loaded.items():
    if d['is_golden']: golden_targets += 1
...
def draw_stock_card(d, ui_key_prefix, is_portfolio=False, p_data=None):
    ...
    try:
        ...
        st.error(f"❌ 渲染標的 {d.get('code', 'Unknown')} 時發生異常，系統已啟動隔離。")
