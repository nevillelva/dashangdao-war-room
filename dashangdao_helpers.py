...
from warroom_core import (
    ...
    FinMindAPIError, PE_LANDMINE, _finmind_get, _lookup_lagged_revenue,
    _lookup_point_in_time_ttm_eps, calculate_atr, compute_higher_high_low_streak,
    detect_attack_streak_reversal, detect_bollinger_overheat, determine_signal,
    ...
    # 【R98續110第九輪】
    calc_intraday_vwap_from_bars, compute_buyer_seller_branch_diff_proxy,
    evaluate_daytrade_recommendation, evaluate_order_book_pressure,
    evaluate_today_liquidity_by_avg, evaluate_vwap_position,
)

# 【R98續110新增，拆檔第三輪：依賴注入用的佔位符】
# 這幾個是Supabase/SQLite連線物件、FinMind多帳號輪替池——dashangdao.py
# 主畫面script流程裡也會直接用到，初始化沒辦法搬過來，改用依賴注入：
# dashangdao.py在自己完成初始化後，會把真正的值寫進這幾個名字
# (dashangdao_helpers.SQLITE_CONN = 真正的連線...)。Python函式查找全域
# 變數是在「被呼叫的當下」才查找，不是定義時就固定，所以只要注入發生在
# 「這個檔案裡的函式真正被呼叫之前」就完全正確，已用最小範例驗證過這個
# 模式可靠。
SQLITE_CONN = None
...
# 【R98續110第七輪，用遞迴檢查巢狀lambda/內部函式的方式才抓到這批遺漏】
# 之前的稽核方法(dis.get_instructions只看外層code object的LOAD_GLOBAL)
# 沒看到藏在lambda裡的呼叫，例如
# `_smart_cached_call(..., lambda: _fetch_real_stock_data_impl(symbol))`
# ——lambda會被編譯成獨立的code object，不會出現在外層函式的co_names裡。
# 這裡補上USER_STATE_KEY(單純字串常數)跟四個自成一體的單例變數(dict/Lock，
# 跟_SB_CALL_EXECUTOR/_SMART_CACHE_STORE同一種模式，內部函式自己lazy
# 初始化用，不需要依賴注入)。
USER_STATE_KEY = "commander_main"
_EXT_HINT = {}
